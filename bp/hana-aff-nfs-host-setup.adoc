...
NOTE: Os exemplos a seguir mostram um banco de dados SAP HANA com SID=NF2 usando LSS.
...
O próximo exemplo mostra um banco de dados SAP HANA com vários hosts e SID=NF2, usando NFSv4.1 para os sistemas de arquivos de dados e de log e NFSv3 para os sistemas de arquivos  `/hana/shared`, `/lss/shared`, e  `/usr/sap/NF2`. Um tamanho de transferência NFS de 1 MB para leituras e 256k para gravações é utilizado.
