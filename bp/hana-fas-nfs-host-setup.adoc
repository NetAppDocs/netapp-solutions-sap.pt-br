...
NOTE: Os exemplos mostram um banco de dados SAP HANA com SID=NF2 usando LSS.

Para criar os subdiretórios necessários, execute uma das seguintes ações:

* Para um sistema de host único, monte o  `/hana/shared` volume e crie os subdiretórios  `shared`,  `lss-shared` e  `usr-sap`.
...
....
* Para um sistema com múltiplos hosts, monte o `/hana/shared` volume e crie os `shared`, `lss-shared` e o `usr-sap` subdiretórios para cada host.
+
...
....
. Execute o comando  `mount -a` para montar os sistemas de arquivos em todos os hosts.


O próximo exemplo mostra um banco de dados SAP HANA com vários hosts e SID=NF2, usando NFSv4.1 para os sistemas de arquivos de dados e log e NFSv3 para os  `/hana/shared`, `/lss/shared` e  `/usr/sap/NF2` sistemas de arquivos. Um tamanho de transferência NFS de 1 MB para leituras e 256k para gravações é utilizado.
