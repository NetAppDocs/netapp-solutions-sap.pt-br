...
NOTE: Os sistemas SAP HANA com múltiplos hosts exigem o sistema de arquivos  `/hana/shared` e os sistemas de arquivos  `/lss/shared`, caso o serviço SAP Local Secure Store (LSS) seja utilizado, conectados a todos os hosts de um sistema. Normalmente, isso é um compartilhamento NFS fornecido por um servidor NFS. Recomenda-se usar um servidor NFS altamente disponível, por exemplo, como um sistema NetApp FAS ou AFF. Outra opção é usar o servidor NFS integrado de um host LINUX para isso.
