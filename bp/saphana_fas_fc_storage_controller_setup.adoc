...
Um conjunto de portas FCP é usado para definir quais LIFs devem ser usados por um grupo específico. Normalmente, todos os LIFs criados para os SISTEMAS HANA são colocados no mesmo conjunto de portas. A figura a seguir mostra a configuração de um conjunto de portas chamado 32g, que inclui os quatro LIFs que já foram criados.

image:saphana_fas_fc_image16.jpeg["Figura que mostra a caixa de diálogo de entrada/saída ou que representa o conteúdo escrito"]
