...
== Proteção autônoma contra ransomware ONTAP

Os snapshots criados pela ONTAP Autonomous Ransomware Protection não podem ser usados para recuperação futura do banco de dados SAP HANA. Portanto, não é recomendável usá-los para volumes de dados e logs do SAP HANA e deve ser desativado para esses volumes. Use a proteção de dados baseada em SnapCenter com snapshots à prova de adulteração para proteção contra ransomware: link:../backup/hana-sc-generic-scope.html["Backup e recuperação do SAP HANA com o SnapCenter"]. ONTAP Autonomous Ransomware Protection pode ser benéfico para outros volumes, como /hana/shared.

É possível desativar a ativação automática desse recurso durante a criação de volumes para um ONTAP Cluster, porém recomenda-se manter a configuração padrão e desativá-lo para volumes onde essa funcionalidade não é necessária.
...
NOTE: Se o serviço SAP Local Secure Store (LSS) for utilizado, o diretório  `/lss/shared/SID`, o diretório de instalação padrão do LSS, estará no disco local. Em uma configuração de recuperação de desastres com replicação baseada em disco, NetApp recomenda criar um diretório dentro do volume  `SID_shared` para o diretório  `/lss/shared/SID` para que todos os sistemas de arquivos estejam no armazenamento central.
...
NOTE: Se o serviço SAP Local Secure Store (LSS) for utilizado, o `/lss/shared/SID` diretório deve ser acessível em todos os hosts. NetApp recomenda criar um diretório dentro do `SID_shared` volume para o `/lss/shared/SID` diretório.
...
| SID_shared | lss-shared | /lss/shared/SID | Montado em todos os hosts 
...
| Desativar cópias Snapshot automáticas | vol modify -vserver <vserver-name> -volume <volname> -snapshot-policy none 


| Desative a proteção contra ransomware, exceto para o volume SID_shared | vol modify -vserver <vserver-name> -volume <volname> -anti-ransomware-state desativado 
