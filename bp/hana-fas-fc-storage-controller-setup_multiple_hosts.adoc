...
O `/hana/shared` volume deve ser acessível a todos os hosts HANA e, portanto, é exportado usando NFS. Embora não existam KPIs de desempenho específicos para o `/hana/shared` sistema de arquivos, NetApp recomenda o uso de uma conexão Ethernet de 10Gb.
...
NOTE: Se o serviço SAP Local Secure Store (LSS) for utilizado, o  `/lss/shared/SID` diretório deve ser acessível em todos os hosts. NetApp recomenda criar um volume para o  `/lss/shared/SID` diretório e montá-lo em todos os hosts usando NFS.
...
Os LUNs de um grupo de volume LVM devem ser armazenados em um agregado diferente e em um controlador diferente. A tabela a seguir mostra um exemplo com dois LUNs por grupo de volume para um sistema SAP HANA com vários hosts 2+1.


NOTE: Não é necessário usar o LVM para combinar vários LUNs para atender aos KPIs do SAP HANA, mas é recomendado.
...
NOTE: O exemplo abaixo define essas opções durante a criação do volume
