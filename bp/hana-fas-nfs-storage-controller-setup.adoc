...
Os Snapshots criados pela ONTAP Autonomous Ransomware Protection não podem ser usados para recuperação futura do banco de dados SAP HANA. Portanto, não é recomendável usá-la para volumes de dados e logs do SAP HANA e deve ser desativada para esses volumes. Use a proteção de dados baseada em SnapCenter com Snapshots à prova de adulteração para proteção contra ransomware: link:../backup/hana-sc-generic-scope.html["Backup e recuperação do SAP HANA com o SnapCenter"]. ONTAP Autonomous Ransomware Protection pode ser benéfica para outros volumes, como /hana/shared.
...
| SID_shared | lss-shared | /lss/shared/ | Montado em todos os hosts 
