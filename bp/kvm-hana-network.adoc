...
summary: Configure interfaces de rede SR-IOV no SUSE KVM para SAP HANA usando placas de rede Mellanox ConnectX.  Configure funções virtuais, atribua-as a máquinas virtuais e configure conexões de rede redundantes para obter desempenho ideal e acesso ao armazenamento. 
---
...
NOTE: Cada VF atribuída a uma VM requer pelo menos 10 Gbit/s de largura de banda.  Não crie nem atribua mais de duas VFs para uma porta física de 25GbE.
...
. Crie um arquivo de unidade systemd `/etc/systemd/system/after.local` com o seguinte conteúdo:
+
....
[Unit]
...
....
. Crie o script _/etc/init.d/after.local_:
+
....
#! /bin/sh
#
...
echo 4 > /sys/class/infiniband/mlx5_1/device/sriov_numvfs
....
. Certifique-se de que o arquivo possa ser executado:
+
....
# cd /etc/init.d/
...
Atribua as funções virtuais criadas à VM do SAP HANA como dispositivos de host PCI usando o _virt-manager_.

. Inicie o virt-manager.
+
image:kvm-hana-image2.png["largura=601, altura=385"]

. Abra a máquina virtual desejada.
+
image:kvm-hana-image3.png["largura=300, altura=385"]

. Selecione *Adicionar hardware*. + image:kvm-hana-image4.png["Virtmanager Adicionar Hardware, largura=601, altura=443"]
. Selecione a placa de rede virtual desejada na primeira porta física da lista de Dispositivos Host PCI e clique em Concluir.
+
...
+
image:kvm-hana-image5.png["largura=601, altura=442"]

. Escolha a próxima porta NIC virtual da lista de Dispositivos Host PCI, use uma porta virtual da segunda porta física e selecione *Concluir*.
+
image:kvm-hana-image6.png["largura=601, altura=441"]
...
image:kvm-hana-image8.png["largura=601, altura=335"]



== O que vem a seguir?

Após configurar as interfaces de rede SR-IOV,link:kvm-hana-fcn.html["Configurar rede Fibre Channel"] se o FCP for usado como protocolo de armazenamento.
