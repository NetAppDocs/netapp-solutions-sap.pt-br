...
---
= Automatização das operações de cópia do sistema SAP com Libelle SystemCopy
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Embora os sistemas de reparo e os sistemas de teste de recuperação de desastres sejam normalmente fornecidos usando clones de sistemas SAP (que não exigem extensas operações de pós-processamento), para sistemas de teste e treinamento atualizados, essas etapas de pós-processamento devem ser aplicadas para permitir a coexistência com o sistema de origem. Portanto, este documento se concentra em cenários de atualização de sistemas SAP. Mais detalhes sobre os diferentes casos de uso podem ser encontrados no relatório técnico link:sc-copy-clone-introduction.html["Automatizando as operações de clonagem e cópia do sistema SAP HANA com o SnapCenter"^].
