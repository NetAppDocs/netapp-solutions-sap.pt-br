...
A corrupção lógica pode ser causada por erros de software, erros humanos ou sabotagem. Infelizmente, a corrupção lógica geralmente não pode ser resolvida com soluções padrão de alta disponibilidade e recuperação de desastres. Como resultado, dependendo da camada, aplicação, sistema de arquivos ou armazenamento onde a corrupção lógica ocorreu, os requisitos máximos de tempo de inatividade e perda de dados podem, às vezes, não ser atendidos.
...
https://www.netapp.com/pdf.html?item=/media/8584-tr4646pdf.pdf["Recuperação de desastres do SAP HANA com replicação de storage"]

link:../backup/hana-dr-anf-data-protection-overview.html["Recuperação de desastres do SAP HANA com o Azure NetApp Files"]
