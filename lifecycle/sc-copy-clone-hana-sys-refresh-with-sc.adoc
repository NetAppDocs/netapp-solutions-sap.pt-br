...
|  | Criptografia de persistência do SAP HANA | SFSS: Inicialmente, ou se as chaves raiz tiverem sido alteradas no sistema de origem, backup(s) da(s) chave(s) raiz devem ser importados no sistema de destino antes que a recuperação possa ser executada. LSS: inicialmente, ou se a senha do backup da chave raiz tiver sido alterada, a senha deve ser definida na configuração do LSS. 
...
|  | Criptografia de persistência do SAP HANA | SFSS: Se um DBID do sistema de origem não existir previamente no sistema de destino, o banco de dados do sistema deverá ser recuperado primeiro, antes que o backup da chave raiz deste tenant possa ser importado. LSS: A topologia LSS será sincronizada automaticamente com a topologia HANA durante a recuperação do(s) banco(s) de dados do tenant. 
...
Em uma configuração SAN Fibre Channel, você precisa fornecer o protocolo utilizado na próxima tela.
