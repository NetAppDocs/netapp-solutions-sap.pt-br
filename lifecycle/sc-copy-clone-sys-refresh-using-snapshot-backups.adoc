...
Quando a criptografia de persistência do SAP HANA está habilitada e, dependendo do armazenamento seguro usado para as chaves raiz de criptografia, instância SFSS ou armazenamento seguro local LSS, etapas adicionais são necessárias antes que você possa recuperar o banco de dados SAP HANA no sistema de destino.

Com o SFSS, as chaves raiz são armazenadas no sistema de arquivos e devem ser copiadas separadamente, além do banco de dados HANA. Os backups das chaves raiz devem então ser importados no sistema de destino para permitir a recuperação do banco de dados HANA. Isso precisa ser feito apenas uma vez, se as chaves raiz não forem alteradas.

Com o LSS, as chaves raiz são copiadas e restauradas juntamente com o banco de dados HANA em uma única operação. Para poder recuperar o banco de dados HANA, é necessário ter a senha que foi usada para o backup das chaves raiz.

As operações necessárias são descritas com mais detalhes nos capítulos seguintes.



=== Armazenamento seguro SFSS
...
=== Armazenamento seguro LSS


NOTE: É necessário usar pelo menos a versão 2.00.088 do HANA, caso contrário, o procedimento descrito não funcionará, veja também a nota SAP 3613110.

Para recuperar um sistema HANA que utiliza criptografia e LSS, você precisa da senha que foi usada para fazer backup das chaves raiz.
...
....
Pode ser usado para definir a senha do sistema de origem na configuração LSS do sistema de destino. A mesma instrução deve ser usada para o sistema e o banco de dados do locatário.

Caso a senha não tenha sido definida previamente, você pode fornecer a senha durante a operação de recuperação.
