...
Este documento descreve a opção de configuração mais comum do SnapCenter para um sistema SAP HANA MDC de host único com um único locatário no FSx para ONTAP. Outras opções de configuração são possíveis e, em alguns casos, necessárias para sistemas SAP HANA específicos, por exemplo, para um sistema com múltiplos hosts. Para obter uma descrição detalhada sobre outras opções de configuração, consulte link:hana-sc-generic-concepts.html["Conceitos e melhores práticas da SnapCenter (NetApp.com)"^].
