...
Foi criado um usuário de banco de dados “SnapCenter” para o banco de dados do sistema HANA com os privilégios necessários para executar operações de backup e recuperação (consulte link:hana-sc-generic-scope.html["Backup e recuperação do SAP HANA com o SnapCenter"^]). Uma chave de armazenamento de usuário HANA deve ser configurada em ambos os hosts usando o usuário de banco de dados acima.
...
As operações de restauração e recuperação para uma configuração de grupo de recursos do SnapCenter 4.6 são idênticas a uma configuração de replicação não-sistema descoberta automaticamente. Todas as opções de restauração e recuperação automática estão disponíveis. Para obter mais detalhes, consulte o relatório técnico. link:hana-sc-generic-scope.html["Backup e recuperação do SAP HANA com o SnapCenter"^].
