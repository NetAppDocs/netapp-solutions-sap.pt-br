...
+
O cenário padrão para sistemas HANA MDC de locatário único é usar o recurso completo com reversão de volume. Para um sistema HANA MDC com vários tenants, talvez você queira restaurar apenas um único tenant. Para obter mais informações sobre a restauração de locatário único, consulte link:hana-sc-generic-restore.html["Restauração e recuperação (NetApp.com)"^].
