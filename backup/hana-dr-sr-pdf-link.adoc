...
[role="lead"]
Este documento fornece uma visão geral das opções de proteção contra desastres para SAP HANA. Inclui informações detalhadas de configuração e uma descrição de caso de uso de uma solução de recuperação de desastres em três sites, baseada em replicação de armazenamento NetApp SnapMirror síncrona e assíncrona. A solução descrita utiliza NetApp SnapCenter com o plug-in SAP HANA para gerenciar a consistência do banco de dados.
