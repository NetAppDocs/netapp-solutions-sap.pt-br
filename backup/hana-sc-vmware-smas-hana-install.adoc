...
Uma descrição mais detalhada pode ser encontrada em link:hana-sc-generic-sc-resource-config.html#sap-hana-backup-user-and-sap-hana-user-store-configuration["Configuração específica de recursos do SnapCenter para backups de bancos de dados SAP HANA"]
