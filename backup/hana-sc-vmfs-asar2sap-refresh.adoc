...
Informações detalhadas sobre as operações de atualização do sistema SAP usando SnapCenter podem ser encontradas em  https://docs.netapp.com/us-en/netapp-solutions-sap/lifecycle/sc-copy-clone-introduction.html["Automatizando as operações de clonagem e cópia do sistema SAP HANA com o SnapCenter"].
...
Para versões do SnapCenter < 6.2, existem limitações que precisam ser consideradas.
...
Conforme mencionado na seção “Pré-requisitos”, para SnapCenter releases < 6.2, o serviço SnapCenter spl no host HANA deve ser reiniciado usando o comando “systemctl restart spl” para iniciar a limpeza adequada.
