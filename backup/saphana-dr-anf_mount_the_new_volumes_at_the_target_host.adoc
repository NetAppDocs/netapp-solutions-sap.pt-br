...
summary: Os novos volumes agora podem ser montados no host de destino, com base no arquivo /etc/fstab criado anteriormente. 
---
...
[role="lead"]
Os novos volumes agora podem ser montados no host de destino, com base no `/etc/fstab` arquivo criado anteriormente.

....
vm-pr1:~ # mount -a
....
A saída a seguir mostra os sistemas de arquivos necessários.
